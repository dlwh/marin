...
Frozen dataclass with cpu/memory/disk/gpu/tpu dimensions. The shared
`check_resource_fit(available, required)` function is used by both
`ScalingGroup.check_resource_fit()` (with `zero_means_unlimited=True`) and
...
CPU jobs get a `device-type=cpu` constraint from `constraints_from_resources()`.
Since every scaling group has CPUs, this constraint would match nothing useful
(no group advertises `device-type=cpu` — they advertise `gpu` or `tpu`).

`routing_constraints()` strips `device-type=cpu` via `is_cpu_device_type_constraint()`.
With that constraint removed, CPU jobs have no device constraints and match all
groups. The waterfall then routes them to the highest-priority group with
capacity.

`ScalingGroup.matches_constraints()` applies the same stripping, so the
per-budget `try_assign()` check is consistent.
