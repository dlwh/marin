...
function workerLink(workerId) {
  if (!workerId) return '-';
  return html`<a href=${'/worker/' + encodeURIComponent(workerId)} class="job-link">${workerId}</a>`;
}
...
function JobRequestDetail({ request }) {
  ...
  const constraints = (request.constraints || []).map(c => {
    const opNames = {
      0: '=', 1: '!=', 2: 'exists', 3: '!exists', 4: '>', 5: '>=', 6: '<', 7: '<=', 8: 'in',
      'CONSTRAINT_OP_EQ': '=', 'CONSTRAINT_OP_NE': '!=', 'CONSTRAINT_OP_EXISTS': 'exists',
      'CONSTRAINT_OP_NOT_EXISTS': '!exists', 'CONSTRAINT_OP_GT': '>', 'CONSTRAINT_OP_GE': '>=',
      'CONSTRAINT_OP_LT': '<', 'CONSTRAINT_OP_LE': '<=', 'CONSTRAINT_OP_IN': 'in',
    };
    const op = opNames[c.op] || c.op || '=';
    const isIn = c.op === 8 || c.op === 'CONSTRAINT_OP_IN';
    if (isIn && c.values && c.values.length > 0) {
      const vals = c.values.map(v => v.stringValue || v.intValue || v.floatValue || '').join(', ');
      return `${c.key} ${op} (${vals})`;
    }
  ...
  return html`
    <${Collapsible} title="Job Request">
      ...
      <${DetailRow} label="CPU" value=${res.cpuMillicores ? (res.cpuMillicores / 1000) : null} />
...
function JobDetailApp() {
  ...
  const [logRegex, setLogRegex] = useState('');
  const [logRegexInput, setLogRegexInput] = useState('');
  const [expandedTasks, setExpandedTasks] = useState(new Set());
  const [profilingTaskId, setProfilingTaskId] = useState(null);
  ...
  async function handleProfileTask(taskId, profilerType, format) {
    setProfilingTaskId(taskId);
    try {
      await profileAndDownload(controllerRpc, taskId, { profilerType, format });
    } catch (e) {
      alert(`${profilerType.toUpperCase()} profile failed: ${e.message}`);
    } finally {
      setProfilingTaskId(null);
    }
  }
  ...
  const fetchTaskLogs = useCallback(async (taskId, tasksList, attemptId = -1) => {
    ...
    try {
      ...
      const params = { id: taskId, maxTotalLines: logLimit };
      if (attemptId >= 0) {
        params.attemptId = attemptId;
      }
      if (logRegex) {
        params.regex = logRegex;
      }
      ...
      setLogsTruncated(!!resp.truncated);
      setLogLineCount(logs.length);
    ...
    async function load() {
      try {
        ...
        const found = {
          ...
          resources: {
            cpu: j.resources ? (j.resources.cpuMillicores || 0) / 1000 : 0,
            memory_bytes: j.resources ? parseInt(j.resources.memoryBytes || 0) : 0,
            disk_bytes: j.resources ? parseInt(j.resources.diskBytes || 0) : 0,
            device: j.resources ? j.resources.device : null,
          },
        };
        ...
        const tasksList = (tasksResp.tasks || []).map(t => ({
          ...
          error: t.error || '',
          pending_reason: t.pendingReason || (stateToName(t.state) === 'pending' ? (j.pendingReason || '') : ''),
          current_attempt_id: t.currentAttemptId || 0,
          resource_usage: t.resourceUsage || null,
  ...
  // Re-fetch logs when the line limit changes (skip the initial render)
  const logLimitInitRef = useRef(true);
  useEffect(() => {
    if (logLimitInitRef.current) {
      logLimitInitRef.current = false;
      return;
    }
    if (selectedTaskId) {
      fetchTaskLogs(selectedTaskId, tasksRef.current, selectedAttemptId);
    }
  }, [logLimit, logRegex, fetchTaskLogs, selectedTaskId, selectedAttemptId]);
  ...
  return html`
    ...
    ${job.pending_reason && html`<div class="scheduling-diagnostic" style="margin-bottom:20px;padding:12px 16px;background:#fff8c5;border:1px solid #d4a72c;border-radius:6px">
      <strong style="color:#9a6700">Scheduling Diagnostic:</strong>
      <pre style="margin:8px 0 0;padding:10px;background:#fffef0;border-radius:4px;font-size:12px;white-space:pre-wrap;font-family:ui-monospace,SFMono-Regular,SF Mono,Menlo,monospace">${job.pending_reason}</pre>
    </div>`}
    ...
    <div class="info-grid">
      ...
      <${InfoCard} title="Resource Request (per VM)">
        ...
        <${InfoRow} label="Accelerator" value=${(() => {
          const base = formatDevice(job.resources.device) || formatDevice(jobRequest?.reservation?.entries?.[0]?.resources?.device);
          const dvConstraint = (jobRequest?.constraints || []).find(c => c.key === 'device-variant' && (c.op === 8 || c.op === 'CONSTRAINT_OP_IN') && c.values);
          if (dvConstraint) {
            const variants = dvConstraint.values.map(v => v.stringValue).join(', ');
            return base ? `${base} (flexible: ${variants})` : `flexible: ${variants}`;
          }
          return base || '-';
        })()} />
    ...
    <div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      <span style="color:#57606a;font-size:13px">Lines:</span>
      ${[100, 1000, 5000, 10000].map(n => html`
        <button
          onClick=${() => setLogLimit(n)}
          style="padding:2px 10px;font-size:12px;border:1px solid ${logLimit === n ? '#0969da' : '#d1d9e0'};border-radius:4px;cursor:pointer;background:${logLimit === n ? '#ddf4ff' : 'white'};color:${logLimit === n ? '#0969da' : '#57606a'}"
        >${n >= 1000 ? (n / 1000) + 'K' : n}</button>
      `)}
      <button
        onClick=${() => setLogLimit(999999)}
        style="padding:2px 10px;font-size:12px;border:1px solid ${logLimit === 999999 ? '#0969da' : '#d1d9e0'};border-radius:4px;cursor:pointer;background:${logLimit === 999999 ? '#ddf4ff' : 'white'};color:${logLimit === 999999 ? '#0969da' : '#57606a'}"
      >All</button>
      ${logsTruncated && html`<span style="color:#9a6700;font-size:12px;font-weight:500">Showing ${logLineCount} lines (truncated)</span>`}
      <span style="color:#57606a;font-size:13px;margin-left:8px">Filter:</span>
      <input
        type="text"
        placeholder="regex"
        value=${logRegexInput}
        onInput=${(e) => setLogRegexInput(e.target.value)}
        onKeyDown=${(e) => { if (e.key === 'Enter') setLogRegex(logRegexInput); }}
        style="padding:2px 8px;font-size:12px;border:1px solid #d1d9e0;border-radius:4px;width:160px;font-family:monospace"
      />
      <button
        onClick=${() => setLogRegex(logRegexInput)}
        style="padding:2px 10px;font-size:12px;border:1px solid #d1d9e0;border-radius:4px;cursor:pointer;background:white;color:#57606a"
      >Apply</button>
      ${logRegex && html`<button
        onClick=${() => { setLogRegex(''); setLogRegexInput(''); }}
        style="padding:2px 10px;font-size:12px;border:1px solid #d1d9e0;border-radius:4px;cursor:pointer;background:white;color:#cf222e"
      >Clear</button>`}
    </div>
      ...
      <tbody>
        ${tasks.flatMap(t => {
          const mainRow = html`<tr key=${t.task_id}>
            <td>${t.task_id}</td>
            <td>${t.task_index ?? '-'}</td>
            <td class=${getStateClass(t.state)}>${t.state}${t.pending_reason ? html`<br/><span class="pending-reason">${t.pending_reason}</span>` : ''}${t.state === 'running' && html`
              <button
                onClick=${() => handleProfileTask(t.task_id, 'cpu', 'SPEEDSCOPE')}
                disabled=${profilingTaskId === t.task_id}
                style="padding:2px 8px;font-size:11px;background:#8250df;color:white;border:none;border-radius:4px;cursor:pointer;margin-left:4px"
                title="CPU profiling with py-spy"
              >
                ${profilingTaskId === t.task_id ? 'Profiling...' : 'CPU'}
              </button>
              <button
                onClick=${() => handleProfileTask(t.task_id, 'memory', 'FLAMEGRAPH')}
                disabled=${profilingTaskId === t.task_id}
                style="padding:2px 8px;font-size:11px;background:#1f883d;color:white;border:none;border-radius:4px;cursor:pointer;margin-left:4px"
                title="Memory profiling with memray"
              >
                ${profilingTaskId === t.task_id ? 'Profiling...' : 'MEM'}
              </button>
            `}</td>
          ...
          const attemptRows = t.attempts.map(a => html`
            <tr key=${t.task_id + '-attempt-' + a.attempt_id} style="background:#f6f8fa">
              ...
              <td style="font-size:12px">${workerLink(a.worker_id)}</td>
              <td></td><td></td>
              <td></td>
