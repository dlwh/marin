...
function AutoscalerLogs() {
  const fetchLogs = (_prefix, limit) =>
    controllerRpc('GetProcessLogs', { prefix: 'iris.cluster.controller.autoscaler', limit: limit || 200 });
  return html`
    <${LogsApp} fetchLogs=${fetchLogs} title="Autoscaler Logs" showControls=${false} />
    ...
    ${(() => {
      ...
      for (const group of groups) {
        for (const slice of (group.slices || [])) {
          for (const vm of (slice.vms || [])) {
            allVms.push({ vm, groupName: group.name, sliceId: slice.sliceId });
          }
        }
      }
      ...
      return html`
        ...
        <table class="scale-groups-table">
          <thead><tr><th>VM</th><th>Group</th><th>Slice</th><th>Address</th><th>State</th></tr></thead>
          <tbody>
            ${allVms.map(({ vm, groupName, sliceId }) => {
              const state = formatVmState(vm.state);
              return html`<tr>
                <td style="font-family:var(--font-mono);font-size:12px">${vm.vmId || '-'}</td>
                <td>${groupName}</td>
                <td style="font-size:12px">${sliceId || '-'}</td>
                <td style="font-family:var(--font-mono);font-size:12px">${vm.address || '-'}</td>
        ...
        <tbody>
          ${(() => {
            ...
            return rows.flatMap(gs => {
              ...
              if (!expanded || !hasSources) return [mainRow];

              const MAX_JOBS = 5;
              const shownJobs = jobRows.slice(0, MAX_JOBS);
              const overflow = jobRows.length - MAX_JOBS;

              const detailRow = html`<tr class="demand-detail-row">
                <td colspan="8">
